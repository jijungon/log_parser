...
| 모듈 | 역할 |
|------|------|
| `lib.rs` | 크레이트 루트 — 모듈을 `pub`로 노출(바이너리·부하도구 공유) |
| `main.rs` | 프로세스 진입점 — 모듈 조립, SIGTERM 처리 |
...
| `process.rs` | 로그 한 줄 공용 처리(strip→정규화→severity→fingerprint→dedup) — coordinator·inbound 공유 |
