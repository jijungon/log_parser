...
> 위치: 4문서 위계의 **생성 주체 (WHO)** — 총괄·구조는 `master-plan.md`, 실행은 `master-plan.md §18`
...
| **stat_report · sos_report 구현 분리** | log_parser Phase B 이후 별도 스텝. 본 프로젝트 범위 | 설계 완료(`master-plan.md §7.8·§7.9`). Agent 2 담당. Phase B 완료 후 구현 |
...
| **MASTER_PLAN + ARCHITECTURE 통합** | observability-design 프레임 채택 | Envelope 스키마 SSoT는 `master-plan.md` §10 |
...
런타임:     tokio (async)
공유 타입:  master-plan.md §10 — Envelope 스키마 (단일 형식, SSoT)
원칙:       master-plan.md §12 (5원칙) 절대 준수
완료 기준:  phase-b.md §1 해당 Step "검증" 항목 통과
참조 문서:  master-plan.md (총괄·구조·요구사항·Envelope 스키마·컴포넌트)
            master-plan.md §18 (작업 순서·산출물·검증 게이트)
```
...
**완료 기준**: phase-b.md §1 Step 1·3 — cgroup self-attach 동작, vector.toml distro 분기 (Ubuntu `/var/log/syslog` / RHEL `/var/log/messages`)

---

## Agent 2 — Snapshot Engineer (stat_report · sos_report)

> **Phase B 이후 구현**. 본 프로젝트 범위. 설계·API 명세는 `master-plan.md §7.8·§7.9` 완료.
...
**완료 기준**: master-plan.md §7.8·§7.9 API 명세 완료. Phase B 이후 별도 Step 검증.
...
**완료 기준**: phase-b.md §1 Step 3 — Vector 60초 운영 RSS <50MB / CPU <1%, cgroup 합산 한도 내, 재시작 후 journald cursor 이어가기
...
**완료 기준**: phase-b.md §1 Step 4 — 동일 메시지 1,000번 → dedup 1건(count=1000), LRU 50,001번째 evict, categories.yaml 룩업, counts 누적 정확
...
**완료 기준**: phase-b.md §1 Step 2·5 — mock에 정상 송신, 단절 5분 누락 0건, Critical burst drop 0건, `/flush` 즉시 응답 + 새 cycle 시작
...
src/envelope.rs          ← Envelope 스키마 구현 (master-plan.md §10을 그대로 옮김)
src/coordinator/
└── envelope.rs          ← cycle 만료/flush 시 envelope 조립 (직렬화 + cycle.seq 단조 증가)
```

**완료 기준**: phase-b.md §1 Step 1·2·4 — cgroup.procs에 PID 등록, 더미 envelope mock 수신, 30분 cycle envelope 1건(단일 schema)
...
공통 데이터 계약:
  Envelope = master-plan.md §10 (단일 형식, event_kind="log_batch")
...
> - **stat_report 구현**: Agent 2. `master-plan.md §7.8` 설계 완료
> - **sos_report 구현**: Agent 2. `master-plan.md §7.9` 설계 완료
