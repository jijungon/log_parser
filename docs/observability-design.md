...
> 본 §은 **무엇을·왜** 분리하는가에 관한 의미 원칙. **어떻게 지키는가의 구현 원칙은 `master-plan.md §12`** 참조.
...
> 구현 세부사항·모듈 위치·완료 기준 → **`agent-roles.md`** / **`master-plan.md §2`** 참조.
...
필드 상세·에이전트별 차이는 `master-plan.md §10` 참조.
...
전체 toml 예시는 `master-plan.md §7.2` (운영 config SSoT). 본 §은 **반드시 지킬 원칙 4개**:
...
> critical만 block을 허용하고 나머지는 drop — 호스트 다른 서비스(journald, logrotate) 영향 차단. 전체 toml 설정은 `master-plan.md §7.2` 참조.

---

## 9. 수신측 책임 (분석 영역)

> 수신측 책임 목록·alerting 룰 예시·schema 진화 정책 → **`receiver-contract.md`** 참조.
...
> **API 명세·envelope 상세·Phase B 검증 계획은 `master-plan.md §7.8·§7.9` 참조.**
...
강건성 메커니즘 상세: `master-plan.md §7.7.1` (disk spool), `§7.11` (process supervision), `§10.1.1` (idempotency · seq 영속화), `§7.4` (dedup LRU 50K 캡).
