...
| 문서 | 내용 |
|------|------|
| **`master-plan.md`** (본 문서) | 전체 구조·컴포넌트·설계 원칙 |
| `phase-b.md` | Phase B 5단계 실행 계획 + 완료 정의 + 용어집 |
| `impl-notes.md` | IPC·Transport·flush·OTLP·RawEvent 구현 세부사항 — **코드 작성 시 참조** |
| `agent-roles.md` | 5 에이전트 책임·구현 계약 |
| `observability-design.md` | 3계층 모델·envelope 스키마 — 설계 근거 참조 |
| `receiver-contract.md` | 수신측 계약 — 송출 contract, 권장 동작, schema 진화 정책 |
| `test-receiver.md` | Phase B 검증용 mock receiver 스펙 (Python, 테스트 전용) |
...
스트림·스냅샷 분리 근거는 `observability-design.md §2 + §12`.
...
- **severity 매핑**: PRIORITY 또는 키워드 기반 (`observability-design.md §6`)
...
- **책임자**: `agent-roles.md` Agent 4 (Pipeline Specialist)
...
- **책임자**: `agent-roles.md` Coordinator (★)
...
> trait·구현체·재시도·spool·`agent.yaml` 상세: `impl-notes.md §2`
...
> 동시성 정책·보안 상세·`agent.yaml`: `impl-notes.md §3`
...
> struct 정의·freeze 정책 상세: `impl-notes.md §5`
...
> Vector source별로 무엇을 수집하고 어떤 문제를 발견할 수 있는지 매핑. 판단은 모두 수신측. `categories.yaml` 기준 파일은 `agent-roles.md` Agent 4.
...
> **JSON 예시·불변 규칙은 `observability-design.md §4` 참조.** 본 §은 운영 명세(필드 의미 표 + 저장 정책 + 송출 규칙)에 집중.
...
> **무엇을·왜의 의미 원칙은 `observability-design.md §1`** 참조.
