...
└── corrupt/         ← 파싱 불가(잘림 등) 파일 격리 (포렌식용, 자동 재전송 안 함, 상한 256MB — 초과 시 오래된 것부터 삭제)
```
...
`retry/`에 쌓인 파일은 두 경로로 재전송됩니다.

- **자동 (기본)**: 라이브 전송이 다시 성공하면(= 수신측 복구 증거) 파서가 `retry/` 전체를 자동 drain합니다 (`transport.auto_drain: true` 기본, 끄기 가능). 장기 장애 후 별도 조치 없이 파킹된 envelope이 회수됩니다.
- **수동**: 수신측 서버가 `POST :9100/drain-spool`로 시간 창을 지정해 재전송. 파일명은 ULID이므로 생성 시각 기준 필터링이 가능합니다. 자동·수동 drain은 같은 가드를 공유해 동시에 돌지 않습니다(중복 발송 없음).
...
| `GET /raw?since&until&sources&max_mb` | 원문 로그 드릴다운(dedup 안 함, bounded, 회전본 포함) | SOS | gzip text (기본 1h/10MB, 상한 24h/30MB) |
...
| **수신측 장기 다운** | 재시도 상한 후 `retry/`에 보관(디스크), 동시 전송 4개 제한 — 파서는 계속 수집하며 **스스로 죽지 않음**. 복구가 감지되면(전송 재성공) `retry/`를 **자동 drain해 파킹분까지 배달** |
...
| **고유 패턴 폭증 (카디널리티)** | SSH 키 지문·커널 주소는 마스킹으로 1종에 수렴. 그래도 dedup 창(5만 종)이 넘치면 축출 그룹을 **폐기 대신 조기 방출**(≤5s) — 집계 유실 없음 |
